...
namespace klib {
	...
	void								drawEntityDetail					(SWeightedDisplay<_Width, _Depth>& display_, int32_t offsetY, int32_t offsetX, const SEntity& entity, const SEntityRecord<_TEntity> (&definitions)[_SizeDefinitions], const SEntityRecord<_TEntity> (&modifiers)[_SizeModifiers], const ::nwol::glabel& entityTypeName)	{
		...
		uint16_t								color;
		sprintf_s(formattedGauge, "%i", entityPoints.Coins						);	printfToGrid(display_.Screen, ++offsetY	, offsetX, nwol::SCREEN_LEFT, formatCoins.c_str()	, "Coins per turn"			, formattedGauge);	valueToGrid(display_.TextAttributes, offsetY, offsetX+23, nwol::SCREEN_LEFT, &(color = (entityPoints.Coins >= 0) ? COLOR_ORANGE : COLOR_RED), 1, 11);												
		sprintf_s(formattedGauge, "%i", entityPoints.PriceBuy/2					);	printfToGrid(display_.Screen, ++offsetY	, offsetX, nwol::SCREEN_LEFT, formatCoins.c_str()	, "Sell Price"				, formattedGauge);	valueToGrid(display_.TextAttributes, offsetY, offsetX+23, nwol::SCREEN_LEFT, &(color = COLOR_ORANGE), 1, 11);																						
		sprintf_s(formattedGauge, "%i", entityPoints.CostMaintenance			);	printfToGrid(display_.Screen, ++offsetY	, offsetX, nwol::SCREEN_LEFT, formatCoins.c_str()	, "Maintenance Cost"		, formattedGauge);	valueToGrid(display_.TextAttributes, offsetY, offsetX+23, nwol::SCREEN_LEFT, &(color = COLOR_ORANGE), 1, 11);
	}
}