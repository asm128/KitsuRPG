...
void selectAITarget(SGame& instanceGame)
{
	...
	// Here we check if the AI has an aggressive flag. If it doesn't just cancel the turn.
	else if( (!nwol::bit_true(playerControl.AIMode, PLAYER_AI_TEAMERS))	
...
void selectAvailableTile(const STacticalBoard& tacticalBoard, nwol::SCoord3<int32_t>& targetPosition)
{
	do 
	{
...
void getValidCoordForAgentDestination(nwol::SCoord3<int32_t>& targetPositionAgent, const STacticalBoard& tacticalBoard)
{
	do 
	{
...
void selectAIDestination(SGame& instanceGame)
{
	...
	nwol::SCoord3<int32_t>&		targetPositionAgent = currentPlayer.Squad.TargetPositions[currentPlayer.Selection.PlayerUnit];
	...
	::nwol::SCoord3<int32_t>	vectorToAgent	= targetPositionAgent-currentPlayer.Army[currentPlayer.Squad.Agents[currentPlayer.Selection.PlayerUnit]]->Position;
	::nwol::SCoord3<float>		vec				= vectorToAgent.Cast<float>();
...
TURN_ACTION selectAIAction(SGame& instanceGame)
{
	...
	else if(0 >= currentPlayer.Squad.ActionsLeft[currentPlayer.Selection.PlayerUnit].Moves && 0 < currentPlayer.Squad.ActionsLeft[currentPlayer.Selection.PlayerUnit].Actions)
	{
		...
		// Here we check if the AI has an aggressive flag. If it doesn't just cancel the turn.
		else if( (nwol::bit_false(currentPlayer.Control.AIMode, PLAYER_AI_TEAMERS))	
			...
			if(currentPlayer.Selection.TargetPlayer != -1 && tacticalInfo.Setup.Players[currentPlayer.Selection.TargetPlayer] != -1) 
			{
				...
				else if(currentPlayer.Selection.TargetUnit != -1 && targetPlayer.Squad.Agents[currentPlayer.Selection.TargetUnit] != -1)
				{
					...
					if(targetCharacter.IsAlive()) 
					{
						...
						const ::nwol::SCoord3<int32_t>&	coordPlayer  = playerAgent.Position; 
						const ::nwol::SCoord3<int32_t>&	coordTarget  = targetCharacter.Position;
						const ::nwol::SCoord3<float>		distance = (coordTarget-coordPlayer).Cast<float>();
