...
int32_t processEquipAgentInput(SGlobalDisplay& display, const nwol::SInput& frameInput, const SPlayer& player, uint32_t rowCount, int32_t offsetY, int32_t offsetX)
{
	...
	for(uint32_t iRow = 0; iRow < rowCount; ++iRow)
	{
		...
		if(mouseOver(mouseX, mouseY, offsetX, offsetY+actualAgentsProcessed, AGENT_ROW_WIDTH)) 
		{
			indexEquip = iRow | (frameInput.MouseButtons[0] ? 0x80000000 : 0);
			break;
		}
...
int32_t drawValueSlider(SGlobalDisplay& display, int32_t offsetY, int32_t offsetX, int32_t value, int32_t labelMaxLen, const ::nwol::glabel& controlLabel )
{
...
,	int32_t offsetY
,	int32_t offsetX
,	const ::nwol::gsyslabel& entityTypeName
...
)
{
	...
	sprintf_s(formattedTitle, " - %s:", entityTypeName.c_str());
...
{
	const ::nwol::gsyslabel& labelet = ::nwol::get_value_label(entityType);
...
{
	const ::nwol::gsyslabel& labelSelectedEquip = ::nwol::get_value_label(entityType);
	switch(entityType)	
	{						
		...
		if(selectedEquip != -1 && selectedAgent == iAgent)
		{
			...
			if(tempRow != -1)
			{
				...
				else //if(tempRow != -1)
				{
					...
					const ::nwol::gsyslabel& labelSelectedEquip = ::nwol::get_value_label(selectedEntityType);
					switch(selectedEntityType)	
					{						
...
SGameState drawWelcome(SGame& instanceGame, const SGameState& returnValue)
{
	...
	const std::string textToPrint = "Welcome back commander " + std::string(instanceGame.Players[PLAYER_INDEX_USER].Name) + ".";
	SGlobalDisplay& display = instanceGame.GlobalDisplay;
