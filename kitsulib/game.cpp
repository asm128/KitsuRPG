...
//GDEFINE_PRINTOBJ(klib, CCharacter, "{%s}", "");
...
void prompt(std::string& userInput, const std::string& displayText) {
	...
	uint32_t screenWidth  =	::nwol::getASCIIBackBufferWidth(),
			 screenHeight =	::nwol::getASCIIBackBufferHeight();

	::nwol::lineToRect(::nwol::getASCIIBackBuffer(), screenWidth, screenHeight, (screenHeight>>1)-1, 0, ::nwol::SCREEN_CENTER, displayText.c_str());
	...
	for(uint32_t iPlayer=0, count=1/*MAX_PLAYER_TYPES*/; iPlayer < count; ++iPlayer) {
		...
		for(uint32_t iAgent=0, agentCount=CAMPAIGN_AGENT_COUNT*2; iAgent<agentCount; iAgent++) {
			...
			if(iPlayer == 0) {
				...
				case 13:
				case 14:
					adventurer.Flags.Tech.Gender = GENDER_FEMALE;
				}
				...
				completeAgentResearch(adventurer);
