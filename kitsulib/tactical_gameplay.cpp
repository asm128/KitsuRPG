...
bool klib::isTacticalValid(SGame& instanceGame)
{
	...
	for(uint32_t iPlayer=0, playerCount = tacticalInfo.Setup.TotalPlayers; iPlayer<playerCount; ++iPlayer ) 
	{
		...
		for(uint32_t iPlayerOther=iPlayer+1; iPlayerOther < playerCount; ++iPlayerOther)
		{
			...
			// Only team AI is considered for the victory
			if(playerOther.Control.Type == PLAYER_CONTROL_AI && playerOther.Control.AIMode != PLAYER_AI_TEAMERS)
				continue;
...
{
	//const SEntityPoints	& targetFinalPoints	= deadTarget.FinalPoints	;
	//const SEntityFlags	& targetFinalFlags	= deadTarget.FinalFlags		;
