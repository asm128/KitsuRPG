...
void addAccessory	(SPlayer& player, const SAccessory	& entity, std::string&	 messageSuccess) { completeProduction(player.Goods.Inventory.Accessory	, entity, messageSuccess, definitionsAccessory	, modifiersAccessory	); }
void addArmor		(SPlayer& player, const SArmor		& entity, std::string&	 messageSuccess) { completeProduction(player.Goods.Inventory.Armor		, entity, messageSuccess, definitionsArmor		, modifiersArmor		); }
void addWeapon		(SPlayer& player, const SWeapon		& entity, std::string&	 messageSuccess) { completeProduction(player.Goods.Inventory.Weapon		, entity, messageSuccess, definitionsWeapon		, modifiersWeapon		); }
void addVehicle		(SPlayer& player, const SVehicle	& entity, std::string&	 messageSuccess) { completeProduction(player.Goods.Inventory.Vehicle	, entity, messageSuccess, definitionsVehicle	, modifiersVehicle		); }
void addProfession	(SPlayer& player, const SProfession	& entity, std::string&	 messageSuccess) { completeProduction(player.Goods.Inventory.Profession	, entity, messageSuccess, definitionsProfession	, modifiersProfession	); }
void addFacility	(SPlayer& player, const SFacility	& entity, std::string&	 messageSuccess) { completeProduction(player.Goods.Inventory.Facility	, entity, messageSuccess, definitionsFacility	, modifiersFacility		); }
void addStageProp	(SPlayer& player, const SStageProp	& entity, std::string&	 messageSuccess) { completeProduction(player.Goods.Inventory.StageProp	, entity, messageSuccess, definitionsStageProp	, modifiersStageProp	); }
...
void handleProductionStep(SGame& instanceGame)
{
	...
	for(size_t iProduct=0, productCount=playerProjects.QueuedProduction.size(); iProduct < productCount; ++iProduct)
	{
		...
		int32_t assignedFunds	= nwol::min(budgetProduction-actualCostProduction, product.PriceUnit-product.PricePaid);
...
void completeResearch( const SEntityResearch& product, SCharacterResearch& playerResearch, std::string& successMessage )
{
	switch(product.Type)
	{
	case ENTITY_TYPE_ACCESSORY	: completeResearch(product, playerResearch.Accessory	.MaxResearch, playerResearch.Accessory	, successMessage); break;
	case ENTITY_TYPE_ARMOR		: completeResearch(product, playerResearch.Armor		.MaxResearch, playerResearch.Armor		, successMessage); break;
	case ENTITY_TYPE_WEAPON		: completeResearch(product, playerResearch.Weapon		.MaxResearch, playerResearch.Weapon		, successMessage); break;
	case ENTITY_TYPE_VEHICLE	: completeResearch(product, playerResearch.Vehicle		.MaxResearch, playerResearch.Vehicle	, successMessage); break;
	case ENTITY_TYPE_PROFESSION	: completeResearch(product, playerResearch.Profession	.MaxResearch, playerResearch.Profession	, successMessage); break;
	case ENTITY_TYPE_FACILITY	: completeResearch(product, playerResearch.Facility		.MaxResearch, playerResearch.Facility	, successMessage); break;
	case ENTITY_TYPE_STAGE_PROP	: completeResearch(product, playerResearch.StageProp	.MaxResearch, playerResearch.StageProp	, successMessage); break;
	//case ENTITY_TYPE_ITEM		: 
	}
